...
void setup() {
    ...
    ManejoEEPROM::cargarRedesEEPROM();

     if(configLora.WiFi) {
        if(!ManejoEEPROM::conectarRedFavorita()) {
            imprimirSerial("No se pudo conectar a red favorita o no configurada", 'y');
        }
    }
    configurarDisplay(configLora.displayOn);
    pinMode(BOTON_MODO_PROG, INPUT_PULLUP); // Inicializacio para activar el modo prog desde boton fisico
...
void loop() {
    vTaskDelay(100 / portTICK_PERIOD_MS);
    modoprogporbotonfisico(); //Se manda a llamar desde interfaz.cpp
}